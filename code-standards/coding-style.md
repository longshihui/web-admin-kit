...
- 禁止在业务逻辑中直接使用含义不明的魔法数、魔法字符串或硬编码状态值。
- 允许直接使用语义显然的字面量，例如 `0`、`1`、`true`、`false`、空字符串和简单数组下标。
- 有业务含义的数字、字符串、状态码、枚举值、时间间隔、分页大小、阈值应抽成具名常量或枚举。
- 常量命名应表达业务含义，不使用 `NUM_ONE`、`STR_TYPE` 这类只描述类型或数值的名称。
- 只在单文件内部使用的常量放在当前文件顶部；跨文件复用的常量放到对应模块的 `constants` 中。
