...
- 菜单构建逻辑响应式、可测试、可复用。
- 菜单组件消费构建后的菜单树，并通过业务项目声明的点击策略处理跳转。
...
定义菜单模型、菜单目标类型常量、active 规则和校验结果。
...
共享包提供 `createMenuClickHandler` 封装通用点击条件和目标类型分发，但不定义固定跳转实现。业务项目通过必填策略声明不同目标类型的执行逻辑：

- `route`：调用 `router.push(location)`。
- `external`：根据业务安全策略打开外链。
- `action`：调用业务项目自己的动作注册或命令处理逻辑。

禁用菜单和无目标分组菜单会直接跳过，不执行业务策略。
