/**
 * 电话号码区号枚举
 * 来源: https://github.com/jjeejj/CountryCodeAndPhoneCode/blob/master/all/countryCodeAndPhoneCode.json
 */
 ...
 * 国际电话编号选项列表
 */
export const PhoneAreaOptions = PhoneArea.map((area) => ({
  label: area.chinese_name,
  value: area.phone_code,
}))

/**
 * 国际电话编号选项列表文案是label + code的形式()
 */
export const PhoneAreaWithCodeOptions = PhoneArea.map((area) => ({
  label: `${area.chinese_name}(+${area.phone_code})`,
  value: area.phone_code,
}))
 ...
 * @param code 区域编码
 */
export function translateAreaCode(code: string) {
  return PhoneArea.find((area) => area.phone_code === code)?.chinese_name
}
